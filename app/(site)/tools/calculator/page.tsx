    ...
    <div className="max-w-6xl mx-auto px-6 md:px-8 py-12 md:py-16">
      ...
      {mode === 'monthly' && (
        <div className="max-w-xl">
          ...
          {/* Total */}
          <div className="mb-6">
            <p className="text-walnut text-sm mb-1">Monthly total</p>
            <p className="font-display text-5xl font-bold text-terracotta">
              ...
              <span className="text-lg font-body font-normal text-walnut ml-2">/month</span>
            </p>
          </div>
          ...
          <div className="mb-8">
            <select
              ...
              className="px-4 py-2 rounded-full border border-sand bg-cream text-espresso text-sm focus:outline-none"
            >
            ...
            {currency !== 'EUR' && !rates && (
              <span className="text-xs text-walnut ml-3">Loading rates...</span>
            )}
          </div>
              ...
              return (
                <div key={item.key}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm font-medium text-espresso">{formatAmt(amount)}</span>
                  </div>
      ...
      {mode === 'compare' && (
        <div className="max-w-2xl">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-8">
            <span className="text-sm text-walnut">Compare Brussels with</span>
            <select
              value={compareCity}
          ...
          {/* Savings headline */}
          <div className="mb-6 p-5 bg-ivory border border-sand/50 rounded-2xl">
            {savings > 0 ? (
          ...
          {/* Table */}
          <div className="border border-sand/50 rounded-2xl overflow-hidden">
            <div className="grid grid-cols-4 bg-sand/30 px-5 py-2">
              <span className="text-xs text-walnut uppercase tracking-wide">Category</span>
              <span className="text-xs text-walnut uppercase tracking-wide text-right">{compareCity}</span>
              <span className="text-xs text-walnut uppercase tracking-wide text-right">Brussels</span>
              <span className="text-xs text-walnut uppercase tracking-wide text-right">Diff</span>
            </div>
              ...
              {(Object.keys(BRUSSELS_COSTS) as (keyof typeof BRUSSELS_COSTS)[]).map(key => {
                ...
                return (
                  <div key={key} className="grid grid-cols-4 px-5 py-3">
                    <span className="text-sm text-espresso capitalize">{key}</span>
                    <span className="text-sm text-espresso text-right">{formatAmt(city)}</span>
                    <span className="text-sm text-espresso text-right">{formatAmt(bru)}</span>
                    <span className={cn(
                      'text-sm font-medium text-right',
                      diff > 0 ? 'text-sage' : diff < 0 ? 'text-coral' : 'text-walnut'
                    )}>
            ...
            <div className="grid grid-cols-4 bg-sand/30 px-5 py-3 border-t border-sand/50">
              <span className="text-sm font-semibold text-espresso">Total</span>
              <span className="text-sm font-semibold text-espresso text-right">{formatAmt(cityTotal)}</span>
              <span className="text-sm font-semibold text-espresso text-right">{formatAmt(brusselsTotal)}</span>
              <span className={cn(
          ...
          <p className="text-xs text-walnut mt-4">
